...
const Tabs = ({ tabs, activeTab, setActiveTab, closeModal }) => {
    const tabClass = (tabId, index) => {
        ...
        const borderClass = activeTab === tabId ? 'border-b-2 border-transparent' : 'border-b-2 border-gray-300 hover:border-gray-500';

        return `${baseClasses} ${activeTab === tabId ? activeClasses : inactiveClasses} ${roundedClass} ${borderClass}`;
    };
